                               ...
                               struct buffer_head *bitmap_bh, unsigned long *count)
{
	...
	while (first_free_bit < nblocks && num < *count) {

    	// Check if the next block is free, if so, increment allocation count
    	if (find_next_zero_bit_le(bitmap_bh->b_data, nblocks, first_free_bit) == first_free_bit + num) {
        	num++;

    	} else {
        	break;  // Stop if consecutive blocks are broken
    	}
	}
	// Mark allocated blocks
	for (unsigned long i = 0; i < num; i++) {
    	ext2_set_bit_atomic(sb_bgl_lock(EXT2_SB(sb), group),first_free_bit + i, bitmap_bh->b_data);
	}

	*count = num;

	if (first_free_bit - group_first_block) return (first_free_bit - group_first_block);
	else return -1;
}
//========================================================================================================
/*
