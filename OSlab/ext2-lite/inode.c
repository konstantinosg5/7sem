                                         ...
                                         struct buffer_head **p)
{
	...
	*p = NULL;
	...
	/* ? */
	block_group = (ino - 1) / inodes_pg;// minus 1 so inodes starting from 1 aline 
					    // with bg starting from 0
	...
	// Start of block groups inode table + offset for the right inode
	...
	*p = bh;
	offset &= (blocksize - 1);
	return (struct ext2_inode *) (bh->b_data + offset);// Blockhead contains the data of the block i want
...
struct inode *ext2_iget(struct super_block *sb, unsigned long ino)
{
	...
	if (S_ISREG(inode->i_mode)) {
		/* ? */
		inode->i_op = &ext2_file_inode_operations; // Regular file inode operations
    		inode->i_fop = &ext2_file_operations;      // File operations for reading/writing
    		inode->i_mapping->a_ops = &ext2_aops;      // Address space operations for file
	} else if (S_ISDIR(inode->i_mode)) {
		/* ? */
		inode->i_op = &ext2_dir_inode_operations; // Directory inode operations
	    	inode->i_fop = &ext2_dir_operations;     // Directory file operations
		inode->i_mapping->a_ops = &ext2_aops;      // Address space operations for file
