                             ...
                             struct folio **foliop)
{
	...
	printk(KERN_DEBUG "FIND ENTRY:entering for loop\n");
	/* Scan all the pages of the directory to find the requested name. */
	for (i=0; i < npages; i++) {
		/* ? */
                kaddr = ext2_get_folio(dir, i, 0, foliop);// getting page by page addr
		if (IS_ERR(kaddr))
	...
	out:
	printk(KERN_DEBUG "FIND ENTRY: FAILED\n");
	return ERR_PTR(-ENOENT);

	pageFound:
	printk(KERN_DEBUG "FIND ENTRY: pageFound!\n");
	return de;
